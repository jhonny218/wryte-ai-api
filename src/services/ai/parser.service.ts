export class ParserService {
  ...
  parseOutlineResponse(response: string | null): any {
    if (!response) return null;

    try {
      // Attempt to find JSON object in the string if it contains extra text
      const start = response.indexOf('{');
      const end = response.lastIndexOf('}');
      ...
      console.error('Failed to parse outline response:', error, response);
      // Fallback: return null so callers can handle gracefully
      return null;
    }
  }

  parseBlogResponse(response: string | null): any {
    if (!response) return null;
    ...
    try {
      ...
      console.error('Failed to parse blog response:', error, response);
      // Fallback: return null so callers can handle gracefully
      return null;
    }
  }
